...
- Интеграционные тесты MCP-сервера как процесса (stdio/smoke test)
- Тесты CLI/install flow для `src/index.ts`, `scripts/setup-machine.sh`, `scripts/run-mcp-server.sh`
- Поддерживать `PLAN.md` как актуальный backlog, а не как первоначальный черновой план
...
Результат: библиотека `core/` с тестовым покрытием, работающая с обычной папкой на диске. Статус: выполнено.
...
Результат: рабочий MCP-сервер, который можно подключить к opencode/Claude Desktop. Статус: выполнено.
...
Результат: полностью рабочий трекер с синхронизацией. Статус: выполнено.

### Фаза 4 — Init-команда

Цель: одна команда создаёт готовый репозиторий work-items.

1. `ob-wi-mcp init <path>` — создаёт папку, `git init`, `.schema/types.yaml` с дефолтными типами, начальный коммит. Статус: выполнено.
